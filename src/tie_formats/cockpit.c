...
bool TieCockpitCoverage_Build(const uint8_t* rgba, int width, int height, TieCockpitCoverage* out,
							  TieFormatError* error) {
	if (!out)
		return TieFormat_SetError(error, 90, "invalid coverage output");
	memset(out, 0, sizeof *out);
	AeronImageCoverage coverage = { 0 };
	if (!Aeron_ImageBuildCoverageRgba8(rgba, width, height, &coverage))
		return TieFormat_SetError(error, 90, "image coverage construction failed");
	out->rects = coverage.rects;
	out->count = coverage.count;
	out->image_width = (uint16_t)coverage.width;
	out->image_height = (uint16_t)coverage.height;
	return true;
}
